...
print(f"{bads} would be dropped extra")
...
amplitudes = {event_id:[] for event_id in event_ids}
plt.figure()
for i,event_id in enumerate(event_ids):
      ...
      # Get the amplitudes
      amplitudes[event_id].extend(np.abs(hil).flatten())
...
amplitudes = np.array([np.mean(amplitudes[event_id]) for event_id in event_ids])
...
data_dict = {"in_phase_diff": in_phase}
savemat(folder_path + "/in_phase_diff.mat", data_dict)
data_dict = {"anti_phase_diff": anti_phase}
