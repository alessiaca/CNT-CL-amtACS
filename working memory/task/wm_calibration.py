...
dataFile = open(f'{p_num}_calibration_{sess}.csv', 'w')
...
win = visual.Window(
    ...
    units='height')
clock = core.Clock()
np.random.seed(43) # Change seed in screening session 
...
def get_response(b,trial,same,condition,win,message,cross,gpr,use_serial,gain,clock): 
    ...
    message.draw()    
    win.flip()
    if use_serial: s.write(str.encode(chr(12)))
    core.wait(0.8)
    message.color = 'white'
...
for b in range(n_blocks):
    ...
    for trial in range(n_trials):
        ...
        if use_serial: s.write(str.encode(chr(10)))
        core.wait(0.2)
        ...
        core.wait(0.2)
        win.flip()
        if use_serial: s.write(str.encode(chr(11)))
