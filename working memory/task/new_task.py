...
os.chdir('C:/Users/alessia/Documents/Praktika/Berlin/CL-amtACS/WM-Task/data')

# Set use_pport = True for EEG recording 
use_pport = False
address = 0x0378
if use_pport: pport = windll.LoadLibrary('inpoutx64.dll')
...
dataFile.write('BlockType, BlockID, Trial, Same, Response, TooSlow, ReactionTime\n')
...
def instructions_task(n_blocks):   
    ...
    sleep(1)
    return
    message.text = '''ORIENTATION TASK''' 
    draw_and_wait(message,win)
    message.text = '''As in the first task, %i colored\n \n squares will shortly appear and disappear''' % (len(rects))
    draw_and_wait(message,win)
    message.text = '''In this task you do NOT need\n \n to keep their position and color in mind'''
    draw_and_wait(message,win)
    message.text = '''Then, a GRATING will appear\n \n TILTED to the right or to the left'''
    draw_and_wait(message,win)
    message.text = '''Your TASK is to indicate whether it is\n \n TILTED to the RIGHT OR to the LEFT'''
    draw_and_wait(message,win)
    message.text = '''Press "F" for LEFT\n \n Press "J" for RIGHT '''
    draw_and_wait(message,win)
    message.text = '''For CORRECT choices you WIN %s cents\n \n For WRONG choices you LOSE %s cents''' % (gpr, gpr)
    draw_and_wait(message,win)
    message.text = '''Are the instructions clear?\n\n\n [Y] Proceed [N] Replay'''
...
def get_response(cond,b,same,probe_ori,win,message,cross,gpr,use_pport,gain,clock):   
    ...
    if not key_res:
        ...
        time= key_res[0][1]
        too_slow = False
        if ((cond == 0 and (key=='f' and not same or key=='j' and same)) or 
        (cond == 1 and (key=='f' and probe_ori >= 90 or key=='j' and probe_ori <= 90))):
    ...
    message.draw()    
    win.flip()
    if use_pport: port.Out32(address, 0)
    core.wait(0.8)
    message.color = 'white'
    
    # Save response data 
    dataFile.write('%i, %i, %i, %s, %s, %s, %.3f\n' %(cond, b, trial, same, response, too_slow, time))
    core.wait(0.8)
...
n_phase_shifts = 3
phase_shifts_all_trials = np.repeat([4,5,6],int(n_trials_tot/n_phase_shifts))
np.random.shuffle(phase_shifts_all_trials)
...
for b in range(n_blocks):
    ...
    for trial in range(n_trials):
        ...
        if use_pport: pport.Out32(address, 1)
        core.wait(0.2)
        
        # Delay
        win.flip()
        if use_pport: 
            pport.Out32(address, 2)
            # Send phase shift for trial to pport --> Speed goat --> Stimulator (Start of stimulation)
            pport.Out32(address, phase_shifts_all_trials[trial])
        core.wait(3.0)
        
        # Probe rectangles
        draw_rectangles(rects,colors_all_trials_new[trial],win)
        if use_pport: 
            pport.Out32(address, 3)
            pport.Out32(address, 8) # End of stimulation
        core.wait(0.2)
        win.flip()
                
        gain = get_response(0,b,same_all_trials[trial],-90,win,message,cross,gpr,use_pport,gain,clock)
